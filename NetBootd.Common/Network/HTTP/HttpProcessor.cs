	...
	public class HttpProcessor : IDisposable
	{
		public static HttpRequest GetRequest(MemoryStream data, byte[] rawBytes)
		{
			...
			using (var streamReader1 = new StreamReader(data))
			{
						...
						if (length != -1)
						{
							...
							if (req.Headers.ContainsKey("Content-Length"))
							{
								...
								using (var memoryStream = new MemoryStream(numArray2))
								{
									...
									{
										var str3 = streamReader2.ReadLine().GetString(Encoding.UTF8);
				...
				if (httpCookie != null && NetbootBase.Providers.ContainsKey("User") && !string.IsNullOrEmpty(httpCookie.Value))
				{
					req.User = NetbootBase.Providers["User"].Request(Guid.Parse(httpCookie.Value));
					if (req.User != null)
